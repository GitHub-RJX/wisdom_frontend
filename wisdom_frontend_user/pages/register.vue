  ...
  <div class="main">
    ...
    <div class="sign-up-container">
      ...
      <div class="more-sign">
        <h6>社交帐号直接注册</h6>
        <ul>
          <li><a id="weixin" class="weixin" target="_blank" href="http://localhost:8150//api/ucenter/wx/login">
...
export default {
  ...
  methods: {
    //注册提交的方法
    submitRegister() {
      registerApi.register(this.registerVo)
        .then(response => {
          //提示注册成功
          this.$message({
            type: 'success',
    ...
    //通过输入手机号发送验证码
    getCodeFun() {
      registerApi.sendCode(this.registerVo.mobile)
        .then(response => {
          this.sending = false;
          //调用倒计时的方法
          this.timeDown();
          alert("您的验证码是 " + response.data.data.code + " ，此验证码在5分钟内有效。如非本人操作请忽略此消息，切勿将验证码随意转告他人！");
        })
    },
    //倒计时
    timeDown() {
